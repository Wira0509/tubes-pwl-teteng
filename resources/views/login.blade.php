...
<body class="bg-gradient-to-r from-red-100 via-red-50 to-red-100 flex items-center justify-center min-h-screen">
    <div class="bg-white p-8 rounded-lg shadow-lg border-2 border-red-300 w-full max-w-md transition-shadow duration-300 hover:shadow-2xl">
        <h1 class="text-3xl font-extrabold mb-6 text-red-700 tracking-wide">Form Login</h1>
        ...
        <form action="{{ route('login.post') }}" method="POST">
            ...
            <button type="submit" class="bg-gradient-to-r from-red-600 to-red-700 text-white px-6 py-3 rounded-lg font-semibold shadow-md hover:from-red-700 hover:to-red-800 transition duration-300">Login</button>
        </form>
        <p class="mt-6 text-sm text-gray-600">
            Belum punya akun? <a href="{{ route('register') }}" class="text-red-600 font-semibold underline hover:text-red-800 transition duration-300">Daftar di sini</a>
        </p>
    </div>
</body>
</html>
